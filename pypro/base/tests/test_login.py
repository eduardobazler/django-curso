...
def test_botao_home(resp_home):
    assert_contains(resp_home, 'Entrar')


def test_link_botao_entrar_disponivel(resp_home):
    assert_contains(resp_home, reverse('login'))


@pytest.fixture
def resp_com_usuario_logado(cliente_com_usuario_logado, db):
...
def test_link_botao_indisponivel(resp_com_usuario_logado):
...
def test_link_botao_sair_disponivel(resp_com_usuario_logado):
